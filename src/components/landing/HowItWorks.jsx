...
const steps = [
  { icon: Coins, title: "Acquire VGOLD", desc: "Purchase tokens during an open tranche. Each token represents 1 troy oz of Tier I certified gold." },
  { icon: Lock, title: "Stake into VGOLD SP", desc: "Lock your tokens into the VGOLD Segregated Portfolio. Choose duration (6–36 months) and risk profile for yields from 1.2% to 15%." },
...
export default function HowItWorks() {
  ...
  return (
    <SectionWrapper id="how-it-works" style={{ padding: isMobile ? "60px 20px" : "80px 40px", background: "#FAFAF5" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "repeat(3, 1fr)", gap: 24, position: "relative" }}>
          {/* Connecting line */}
          {!isMobile && (
            <div style={{ position: "absolute", top: 52, left: "20%", right: "20%", height: 2, background: `${LIME}35`, zIndex: 0 }} />
          )}
          {steps.map((step, i) => (
            <motion.div
              key={i}
            ...
            >
              <div style={{ width: 56, height: 56, borderRadius: "50%", background: `${LIME}18`, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 20px", border: `2px solid ${LIME}40` }}>
                <span style={{ fontSize: 22, fontWeight: 700, color: GOLD, fontFamily: "Georgia, serif" }}>{i + 1}</span>
              </div>
              <div style={{ width: 36, height: 36, borderRadius: 8, background: `${LIME}15`, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px" }}>
